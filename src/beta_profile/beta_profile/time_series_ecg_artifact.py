...
def plot_ieeg_data(sub: str, hemisphere: str, condition: str, session: str, group: str):
    """
...
def ecg_cleaning(
    ...
    # find the path to the results folder
    results_path = find_folders.get_local_path(folder="results")
    figures_path = find_folders.get_local_path(folder="figures")
    ...
    for hem in HEMISPHERES:
        ...
        for group in CH_GROUPS:
            ...
            if input_y_or_n_artifact == "n":
                ...
                clean_time_series_dict[f"{sub}_{hem}_{session}_{group}"] = [
                    sub,
                    hem,
                    session,
                    group,
            ...
            plot_ica["fig_ecg_components"].savefig(
                os.path.join(
                    figures_path, f"ica_components_sub-{sub}_{session}_{group}.svg"
                ),
                bbox_inches="tight",
                format="svg",
            )
            plot_ica["fig_ecg_components"].savefig(
                os.path.join(
                    figures_path, f"ica_components_sub-{sub}_{session}_{group}.png"
                ),
                bbox_inches="tight",
            )
            ...
            plot_cleaned = plot_ieeg_data(
                sub, session, group, cleaned_ieeg_data, fig_title="cleaned_time_series"
            )
            ...
            if input_y_or_n_clean == "n":
                ...
                if second_input_y_or_n == "y":
                    ...
                    second_plot_ica["fig_ecg_components"].savefig(
                        os.path.join(
                            figures_path,
                            f"second_run_ica_components_sub-{sub}_{session}_{group}.png",
                        ),
                        bbox_inches="tight",
                    )
