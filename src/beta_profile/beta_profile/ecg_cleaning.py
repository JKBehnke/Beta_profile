...
def plot_ieeg_data(
    sub: str, session: str, channel_group: str, ieeg_data, fig_title: str
):
    """
    ...
    fig.savefig(
        os.path.join(
            figures_path, f"{fig_title}_sub-{sub}_{hem}_{session}_m0s0_{group_name}.svg"
        ),
        bbox_inches="tight",
        format="svg",
    )
    fig.savefig(
        os.path.join(
            figures_path, f"{fig_title}_sub-{sub}_{hem}_{session}_m0s0_{group_name}.png"
        ),
        bbox_inches="tight",
    )
...
def plot_raw_time_series_before_MNE(sub: str, session: str, channel_group: str):
    ...
    fig.suptitle(
        f"raw time series sub-{sub}, {hem} hemisphere", ha="center", fontsize=15
    )
    fig.tight_layout()
    plt.subplots_adjust(wspace=0, hspace=0)
    ...
    fig.savefig(
        os.path.join(
            figures_path,
            f"raw_time_series_sub-{sub}_{hem}_{session}_m0s0_{group_name}.svg",
        ),
        bbox_inches="tight",
        format="svg",
    )
    fig.savefig(
        os.path.join(
            figures_path,
            f"raw_time_series_sub-{sub}_{hem}_{session}_m0s0_{group_name}.png",
        ),
        bbox_inches="tight",
    )
...
def ecg_cleaning(sub: str):
    ...
    sessions = sub_session_dict.get_sessions(
        sub
    )  # list of existing sessions for the subject
    ...
    if perceive_error != "No":
        print(
            f"Perceive error for {sub} in sessions: {perceive_error}."
        )  # alternative code will be used

    # iterate over sessions
    for ses in sessions:

        if ses in perceive_error:
            data_from_json = bssu_json.load_json_data_if_perceive_error(
                sub=sub, session=ses, condition="m0s0"
            )
        ...
        for group in CHANNEL_GROUPS:
            ...
            if ses in perceive_error:
                plot_raw = plot_ieeg_data(
                    sub, ses, group, data_from_json[group], fig_title="raw_time_series"
                )
            ...
            if input_y_or_n_artifact == "n":
                ...
                ecg_artifact_df = pd.concat(
                    [ecg_artifact_df, new_row], ignore_index=True
                )
            ...
            plot_ica["fig_ecg_components"].savefig(
                os.path.join(
                    figures_path, f"ica_components_sub-{sub}_{ses}_{group}.svg"
                ),
                bbox_inches="tight",
                format="svg",
            )
            plot_ica["fig_ecg_components"].savefig(
                os.path.join(
                    figures_path, f"ica_components_sub-{sub}_{ses}_{group}.png"
                ),
                bbox_inches="tight",
            )
            ...
            plot_cleaned = plot_ieeg_data(
                sub, ses, group, cleaned_ieeg_data, fig_title="cleaned_time_series"
            )
            ...
            if input_y_or_n_clean == "n":
                ...
                if second_input_y_or_n == "y":
                    ...
                    second_plot_ica["fig_ecg_components"].savefig(
                        os.path.join(
                            figures_path,
                            f"second_run_ica_components_sub-{sub}_{ses}_{group}.png",
                        ),
                        bbox_inches="tight",
                    )
                    ...
                    if second_input_ecg_component in {1, 2, 3, 4, 5, 6}:
                        ...
                        plot_cleaned = plot_ieeg_data(
                            ...
                            group,
                            cleaned_ieeg_data,
                            fig_title="second_run_cleaned_time_series",
                        )
...
def plot_raw_time_series(incl_sub: list, incl_session: list, incl_condition: list):
    ...
    artifact_dict = (
        {}
    )  # dictionary with tuples of frequency and psd for each channel and timepoint of a subject
            ...
            for t, tp in enumerate(incl_session):
                ...
                for g, group in enumerate(channel_groups):
                    ...
                    for c, cond in enumerate(incl_condition):
                        ...
                        include_channelList = (
                            []
                        )  # this will be a list with all channel names selected
                        ...
                        # pick channels of interest: mne.pick_channels() will output the indices of included channels in an array
                        ch_names_indices = mne.pick_channels(
                            ch_names, include=include_channelList
                        )
                        ...
                        for i, ch in enumerate(ch_names):
                            ...
                            artifact_dict[
                                f"{sub}_{hem}_{tp}_{group_name}_{cond}_{channels[i]}"
                            ] = [
                                sub,
                                hem,
                                tp,
                            ...
                            axes[i].set_title(
                                f"{tp}, {group}, {channels[i]}", fontsize=15
                            )
                            axes[i].plot(
                                signal,
                                label=f"{channels[i]}_{cond}",
                                color="k",
                                linewidth=0.3,
                            )
                        ...
                        fig.suptitle(
                            f"raw time series sub-{sub}, {hem} hemisphere",
                            ha="center",
                            fontsize=15,
                        )
                        fig.tight_layout()
                        ...
                        fig.savefig(
                            os.path.join(
                                figures_path,
                                f"raw_time_series_sub-{sub}_{hem}_{tp}_{cond}_{group_name}.png",
                            ),
                            bbox_inches="tight",
                        )
